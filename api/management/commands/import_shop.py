...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not shop_title:
            self.stdout.write(
                self.style.ERROR('В файле не указано имя магазина (ключ "shop")')
            )
            return

        # Ищем по полю title
        shop, created = Shop.objects.get_or_create(
            title=shop_title, defaults={"accepts_orders": True}
        )

        action_word = "создан" if created else "найден/обновлен"
        self.stdout.write(
            self.style.SUCCESS(f'Магазин "{shop.title}" {action_word}. ID: {shop.id}')
        )

        # 2. Очистка старых предложений этого магазина
        deleted_count, _ = ProductInfo.objects.filter(shop=shop).delete()
        self.stdout.write(
            self.style.SUCCESS(f"Удалено старых предложений: {deleted_count}")
        )
        ...
        for item in data.get("goods", []):
            ...
            cat_data = next(
                (c for c in data.get("categories", []) if c["id"] == cat_id), None
            )
            ...
            category, _ = Category.objects.get_or_create(
                id=cat_data["id"], defaults={"title": cat_data["name"]}
            )
            ...
            existing_params = {
                pp.parameter.name: pp
                for pp in offer.parameter_links.select_related("parameter").all()
            }
            ...
            for param_name, value in item.get("parameters", {}).items():
                ...
                if param_name in existing_params:
                    ...
                    ProductParameter.objects.create(
                        product_info=offer, parameter=parameter_obj, value=str(value)
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ Импорт завершен. Новых предложений создано: {created_offers}"
            )
        )
